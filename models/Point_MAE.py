...
######## 不用FC 来重建，而是用foldnet 来重建
@MODELS.register_module()
class Point_MAE_folding(nn.Module):
    def __init__(self, config):
        super().__init__()
        print_log(f'[Point_MAE_folding] ', logger ='Point_MAE_folding')
        self.config = config
        self.trans_dim = config.transformer_config.trans_dim
        self.MAE_encoder = MaskTransformer(config)
        self.group_size = config.group_size
    ...
    def forward(self, corrupted_pts, pts, vis = False, **kwargs):
        ...
        points = self.build_grid(x_rec.shape[0]).transpose(1,2)  # (batch_size*group number, 2, 36) or (batch_size*group number, 3, 36)
...
class PointTransformer(nn.Module):
    ...
    def forward(self, pts):
        ...
        x = self.norm(x) # B G+1 N
        concat_f = torch.cat([x[:, 0], x[:, 1:].max(1)[0]], dim=-1)  ## B 2N
        ret = self.cls_head_finetune(concat_f)
        return ret

# finetune model without class token
@MODELS.register_module()
class PointTransformerNoClassToken(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.config = config
    ...
    def forward(self, pts):
        ...
        pos = pos
        # transformer
        x = self.blocks(x, pos) # B G N
        x = self.norm(x) # B G N
        concat_f = x.max(1)[0] + x.mean(1)
        # x_vis.mean(1) + x_vis.max(1)[0]
        # concat_f = torch.cat([x[:, 0], x[:, 1:].max(1)[0]], dim=-1)  ## B 2N
        ret = self.cls_head_finetune(concat_f)
        return ret
...
# finetune model
@MODELS.register_module()
class PointTransformerLinearClassification(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.config = config
...
# finetune model for svm classification.
@MODELS.register_module()
class PointTransformerNoClassTokenSVMFeature(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.config = config
