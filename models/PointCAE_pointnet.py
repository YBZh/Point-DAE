...
class Point_CAE_PointNet(nn.Module):
    def __init__(self, config):
        ...
        self.pointnet_encoder = PointNetEncoder(global_feat=True, feature_transform=False, channel=3)
    ...
    def forward(self, corrupted_pts, pts, vis = False, **kwargs):
        ...
        feature, _, _ = self.pointnet_encoder(corrupted_pts)
...
class PointNet(nn.Module):
    ...
    def get_loss_acc(self, ret, gt):
        if self.smoothing:
            eps = 0.3
            n_class = ret['cls'].size()[1]
            one_hot = torch.zeros_like(ret['cls']).scatter(1, gt.long().view(-1, 1), 1)  # (num_points, num_class)
            one_hot = one_hot * (1 - eps) + (1 - one_hot) * eps / (n_class - 1)
            log_prb = F.log_softmax(ret['cls'], dim=1)
            loss_cls = -(one_hot * log_prb).sum(dim=1).mean()  # ~ F.nll_loss(log_prb, gold)

            mat_diff_loss_scale = 0.001
            mat_diff_loss = feature_transform_regularizer(ret['trans_feat'])
            loss = loss_cls + mat_diff_loss_scale * mat_diff_loss
        else:
            mat_diff_loss_scale = 0.001
            loss_cls = self.loss_ce(ret['cls'], gt.long())
            mat_diff_loss = feature_transform_regularizer(ret['trans_feat'])
            loss = loss_cls + mat_diff_loss_scale * mat_diff_loss

        # loss = self.loss_ce(ret, gt.long())
        pred = ret['cls'].argmax(-1)
    ...
    def forward(self, pts):
        ...
        feature, trains, trans_feat = self.pointnet_encoder(pts)
