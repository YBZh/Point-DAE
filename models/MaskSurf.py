...
@MODELS.register_module()
class MaskSurf_estimated_position(nn.Module):
    def __init__(self, config):
        super().__init__()
        print_log(f'[MaskSurfel] ', logger ='MaskSurf')
        self.config = config
    ...
    def forward(self, corrupted_pts, pts, vis = False, return_feat=False, **kwargs):
        ...
        if return_feat:
            ...
            neighborhood, neighborhood_attribute, center = self.group_divider(pts)
            ...
            B, M, C = x_rec.shape
            rebuild_points = self.increase_dim(x_rec.transpose(1, 2)).transpose(1, 2).reshape(B * M, -1, 3)  # BM Gs 3
            rebuild_normal = self.increase_dim2(x_rec.transpose(1, 2)).transpose(1, 2).reshape(B * M, -1, 3)  # BM Gs 3

            gt_points = neighborhood[mask].reshape(B*M,-1,3) ## BM, Gs, 3
            gt_normals = neighborhood_normal[mask].reshape(B * M, -1, 3)  ## BM, Gs, 3
            loss_xyz, loss_normal = self.loss_func(rebuild_points, gt_points, rebuild_normal, gt_normals)

            unnormalized_rebuild_points = rebuild_points + center[mask].unsqueeze(1)
            unnormalized_gt_points = gt_points + center[mask].unsqueeze(1)

            rebuild_position = torch.abs((unnormalized_rebuild_points * torch.nn.functional.normalize(rebuild_normal, dim=2)).sum(-1)) ## BM, Gs
            gt_position = torch.abs((unnormalized_gt_points * torch.nn.functional.normalize(gt_normals, dim=2)).sum(-1)) ## BM, Gs
            position_loss = (rebuild_position - gt_position).pow(2).mean()
            loss_normal = loss_normal + position_loss * self.position_weight
