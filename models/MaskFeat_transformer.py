...
class MaskFeat_transformer_pointmae(nn.Module):
    def __init__(self, config):
        super().__init__()
        print_log(f'[MaskFeat_transformer_pointmae] ', logger='MaskFeat_transformer_pointmae')
        self.config = config
        ...
        self.pointmae = builder.model_builder(config.pointmae_config)
        # self.pointmae = Point_MAE(config.pointmae_config)
        self._prepare_pointmae()

        for param in self.pointmae.parameters():
            param.requires_grad = False

    def _prepare_pointmae(self):
        pointmae_ckpt = self.config.pointmae_config.ckpt
        ckpt = torch.load(pointmae_ckpt, map_location='cpu')
        base_ckpt = {k.replace("module.", ""): v for k, v in ckpt['base_model'].items()}
        self.pointmae.load_state_dict(base_ckpt, strict=True)
        print_log(f'[dVAE] Successful Loading the ckpt for pointmae from {pointmae_ckpt}', logger='Point_BERT')
    ...
    def forward(self, corrupted_pts, pts, vis=False, return_feat = False, **kwargs):
        ...
        if return_feat:
            ...
            if self.all_patch == 'True':
                with torch.no_grad():
                    x_all, _ = self.pointmae.MAE_encoder(neighborhood, center, noaug = True)
            else:
                with torch.no_grad():
                    x_all, _ = self.pointmae.MAE_encoder(neighborhood, center, noaug = True)
