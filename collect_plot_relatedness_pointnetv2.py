...
dic_corruption2name = {
    ...
    'Supervised': "pretrain_supervised_affine_droplocalPointNetv2",
}
...
def plot_relatedness_scanhard(results_shapenet, results_downstream, save_name):
    # # ##################### x with loss
    # results_shapenet[1] = 0.89
    # results_downstream[1] = 1.1
    ################################## x with classification acc
    results_shapenet[1] = 0.741 ## PointNetv2, humanpose
    # results_shapenet[-2] = 0.782 ## PointNetv2, humanpose, Affine+Drop Local
    # results_shapenet[-3] = 0.78 ## PointNetv2, humanpose, Affine+Drop patch
    ...
    # ax.annotate('p = 0.29', xy=(x_max, y_max), \
    #             xytext=(x_max - x_gap * 0.15, y_max), size=35)
    # ax.annotate('p < .001', xy=(x_min, y_max), \
    #             xytext=(x_min, y_max), size=35)
    ax.annotate('r = 0.79', xy=(x_min, y_max), \
                xytext=(x_min, y_max), size=35)
...
plot_relatedness_scanhard(results_shapenet, results_downstream, save_name='shapenet_acc_scanhard_acc_human_pose_pointnetv2')
...
def plot_relatedness_modelnet(results_shapenet, results_downstream, save_name):
    ...
    # ax.annotate('p < .001', xy=(x_min, y_max), \
    #             xytext=(x_min, y_max), size=35)
    ax.annotate('r = 0.87', xy=(x_min, y_max), \
                xytext=(x_min, y_max), size=35)
    ax.annotate('p < .001', xy=(x_min, y_max-0.0025), \
                xytext=(x_min, y_max-0.0025), size=35)
    ax.plot(shapenet_acc_sorted_real, slope * shapenet_acc_sorted_real + intercept, 'k--', linewidth=5)
    plt.tick_params(labelsize=40)
    ax.set_xlabel(r'Task Relatedness', fontsize=30)
    ax.set_ylabel('Acc.(%) on Downstream Tasks', fontsize=30)
    ...
    ax.set_xlim([x_min - x_gap * 0.06, x_max + x_gap * 0.06])
    ax.set_ylim([y_min - y_gap * 0.06, y_max + y_gap * 0.06])
    image_name = save_name + '.pdf'
    f.savefig(image_name)
...
plot_relatedness_modelnet(results_shapenet, results_downstream, save_name='shapenet_acc_modelnet_acc_human_pose_pointnetv2')



# ###############################################################################################
# # shapenet_acc_scanobjbg_acc_human_pose_pointnetv2
# # !!!! results_shapenet[1] = 0.741 ## PointNetv2, humanpose
# ###############################################################################################
# ModelName='PointNetv2_feat'
# shapenet_relatedness_root = './experiments/finetune_shapenet_task_affinity_svm_classification'
...
# results_downstream = collect_results(downstream_task_root, file_list, require_taskaffinity_in_name)
#
# plot_relatedness(results_shapenet, results_downstream, save_name='shapenet_acc_scanobjbg_acc_human_pose_pointnetv2')
...
# results_downstream = collect_results(downstream_task_root, file_list, require_taskaffinity_in_name)
#
# plot_relatedness(results_shapenet, results_downstream, save_name='shapenet_loss_scanhard_loss_human_pose_pointnetv2')
