...
def build_opti_sche(base_model, config):
    ...
    if opti_config.type == 'AdamW':
        def add_weight_decay(model, weight_decay=1e-5, skip_list=()):
            ...
            decay = []
            no_decay = []
            decay_pretrain, decay_new = [], []
            no_decay_pretrain, no_decay_new = [], []
                ...
                if config.optimizer.part == 'only_new': ## only the newly added classifier is learned.
                    if ('cls' in name):
                        if len(param.shape) == 1 or name.endswith(".bias") or 'token' in name or name in skip_list:
                            ...
                            decay.append(param)
                        print(name)
                elif config.optimizer.part == 'diff_lr': ## only the newly added classifier is learned.
                    if ('cls' in name):
                        if len(param.shape) == 1 or name.endswith(".bias") or 'token' in name or name in skip_list:
                            no_decay_new.append(param)
                        else:
                            decay_new.append(param)
                        print('new_added', name)
                    else:
                        if len(param.shape) == 1 or name.endswith(".bias") or 'token' in name or name in skip_list:
                            no_decay_pretrain.append(param)
                        else:
                            decay_pretrain.append(param)
                        print('pre_trained', name)
                elif config.optimizer.part == 'decoder':  ## only the decoder part is optimized, for transformer only.
                    if ('decoder_pos_embed' in name) or ('MAE_decoder' in name) or ('increase_dim' in name):
                        if len(param.shape) == 1 or name.endswith(".bias") or 'token' in name or name in skip_list:
                            ...
                            decay.append(param)
                        print(name)
                elif config.optimizer.part == 'all':  ## fine-tuning all parameter with the same learning rate.
                    if len(param.shape) == 1 or name.endswith(".bias") or 'token' in name or name in skip_list:
                        # print(name)
                        no_decay.append(param)
                    else:
            ...
            if config.optimizer.part == 'diff_lr':
                return [
                    {'params': no_decay_pretrain, 'weight_decay': 0., 'lr': config.optimizer.kwargs.lr * 0.1},
                    {'params': decay_pretrain, 'weight_decay': weight_decay, 'lr': config.optimizer.kwargs.lr * 0.1},
    ...
    num_epochs = sche_config.kwargs.epochs # it seems that this is not used.
    warmup_epochs = getattr(sche_config.kwargs, 'warmup_epochs', 0)
    warmup_lr = getattr(sche_config.kwargs, 'warmup_lr', 1.0e-6)  # linear warmup
    min_lr = sche_config.kwargs.min_lr if getattr(sche_config.kwargs, 'min_lr', False) else opti_config.kwargs.lr/1000.
    t_max = getattr(sche_config.kwargs, 't_max', num_epochs)
