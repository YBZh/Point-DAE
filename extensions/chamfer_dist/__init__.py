...
class ChamferDistanceL2(torch.nn.Module):
    ...
    def forward(self, xyz1, xyz2):
        ...
        # return torch.mean(our_dist1) + torch.mean(our_dist12)


class ChamferDistanceL2_corase2fine(torch.nn.Module):
    f''' Chamder Distance L2
    '''
    def __init__(self, ignore_zeros=False):
        super().__init__()
    ...
    def forward(self, xyz1, xyz2, fine1, fine2):
        ## xyz1, xy2: B * patch_num * 3
        ## fine1, fine2: B * patch_num * patch_size * 3
        batch_size = xyz1.size(0)
        batch_size, patch_numb, patch_size, xyz_length = fine1.shape
        # if batch_size == 1 and self.ignore_zeros:
        #     non_zeros1 = torch.sum(xyz1, dim=2).ne(0)
        #     non_zeros2 = torch.sum(xyz2, dim=2).ne(0)
        #     xyz1 = xyz1[non_zeros1].unsqueeze(dim=0)
        ...
        # ipdb.set_trace()
        ## fine1, fine2 --> (B*patch_num, patch_size, *3)
        ## 关键是找到fine1, fine2 对应的patch.
        target_xyz1 = torch.gather(fine2, 1, idx1.long().unsqueeze(2).unsqueeze(3).expand(fine1.size()))
        target_xyz2 = torch.gather(fine1, 1, idx2.long().unsqueeze(2).unsqueeze(3).expand(fine2.size()))
        our_fine_dist1 = self.cd_loss(fine1.reshape(-1, patch_size, xyz_length).contiguous(), target_xyz1.reshape(-1, patch_size, xyz_length).contiguous()) ## this is identical to dist1
        our_fine_dist2 = self.cd_loss(fine2.reshape(-1, patch_size, xyz_length).contiguous(), target_xyz2.reshape(-1, patch_size, xyz_length).contiguous()) ## this is identical to dist2
        return torch.mean(dist1) + torch.mean(dist2), torch.mean(our_fine_dist1) + torch.mean(our_fine_dist2)


        # target_xyz1 = torch.gather(fine1, 1, idx1.long().unsqueeze(2).expand(normal_rebuild.size()))
        # target_xyz2 = torch.gather(fine2, 1, idx2.long().unsqueeze(2).expand(normal_gt.size()))
...
class ChamferDistanceL2_withnormal(torch.nn.Module):
    ...
    def forward(self, xyz1, xyz2, normal_rebuild, normal_gt, curve_rebuild=torch.rand(0), curve_gt=torch.rand(0), position_rebuild=torch.rand(0), position_gt=torch.rand(0)):
        # xyz1: B * Gs * 3
        batch_size = xyz1.size(0)
        # if batch_size == 1 and self.ignore_zeros:
        #     non_zeros1 = torch.sum(xyz1, dim=2).ne(0)
        ...
        if curve_rebuild.size()[0] != 0:
            ...
            curve_dist1 = dis_l2(curve_rebuild, target_xyz1_curve)
            curve_dist2 = dis_l2(curve_gt, target_xyz2_curve)
            if position_rebuild.size()[0] != 0:
                target_xyz1_position = torch.gather(position_gt, 1, idx1.long().unsqueeze(2).expand(position_rebuild.size()))
                target_xyz2_position = torch.gather(position_rebuild, 1, idx2.long().unsqueeze(2).expand(position_gt.size()))
                position_dist1 = dis_l2(position_rebuild, target_xyz1_position)
                position_dist2 = dis_l2(position_gt, target_xyz2_position)
                return torch.mean(dist1) + torch.mean(dist2), torch.mean(normal_dist1) + torch.mean(
                    normal_dist2), torch.mean(curve_dist1) + torch.mean(curve_dist2), torch.mean(position_dist1) + torch.mean(position_dist2),
            else:
