...
def get_args():
    ...
    parser.add_argument('--root_folder', type=str, default='experiments', help='experiments')
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help = 'test mode for corrupted samples with certain ckpt')
    parser.add_argument(
        '--so3_rotation',
        action='store_true',
        default=False,
        help = 'test mode for so3 rotation with certain ckpt')
    parser.add_argument(
        '--finetune_model', 
        action='store_true', 
        default=False, 
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help = 'evaluate task affinity with pre-trained encoder.')
    parser.add_argument(
        '--scratch_model', 
        action='store_true', 
        default=False, 
    ...
    if args.test_corruption:
        args.exp_name = 'test_corruption_' + args.exp_name
    if args.mode is not None:
        args.exp_name = args.exp_name + '_' +args.mode
    args.experiment_path = os.path.join('./' + args.root_folder, Path(args.config).stem + args.model_name, Path(args.config).parent.stem, args.exp_name)
    args.tfboard_path = os.path.join('./' + args.root_folder, Path(args.config).stem + args.model_name, Path(args.config).parent.stem,'TFBoard' ,args.exp_name)
