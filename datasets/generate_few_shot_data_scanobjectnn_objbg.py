...
target = '../data/ScanObjectNN_objbg_fewshot_v2'
...
def generate_fewshot_data(way, shot, prefix_ind, eval_sample=20):
    ...
    for i, key in enumerate(keys[:way]):
        ...
        else: ## if instance number in selected category is less than the eval_sample, then use all instance number is this category.
            for data in test_data_list[:len(test_data_list)]:
                test_dataset.append((data, i, key))
