...
class ModelNet(Dataset):
    ...
    def _get_item(self, index):
        ...
        point_set[:, 0:3] = augment_data(point_set[:, 0:3], self.aug_type)  # one PC norm is always applied at the beginning of aug.
...
@DATASETS.register_module()
class ModelNet_Official(Dataset):
    def __init__(self, config):
        self.data, self.label = load_modelnet_data(config.subset)
        self.num_points = config.N_POINTS
        self.partition = config.subset
        self.aug_type = config.aug_type
        print(self.num_points) ## 2048.

    def __len__(self):
        return self.data.shape[0]

    def __getitem__(self, index):
        pointcloud = self.data[index][:self.num_points] ### extract all point in the dataset, e.g., 2048
        pointcloud = augment_data(pointcloud, self.aug_type)  # one PC norm is always applied at the beginning of aug.
        # pt_idxs = np.arange(0, pointcloud.shape[0])  # 2048
        # if self.partition == 'train': ### most model is invariant to point shuffle.
        #     np.random.shuffle(pt_idxs)
        # current_points = pointcloud[pt_idxs].copy()
        # current_points = torch.from_numpy(current_points).float()
        current_points = torch.from_numpy(pointcloud).float()
