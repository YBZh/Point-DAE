...
class Objaverse_8192(data.Dataset):
    def __init__(self, config):
        ...
        self.file_list = []
        sub_file_list = os.listdir(self.data_root)
        for sub_file in sub_file_list:
            sub_dir = os.path.join(self.data_root, sub_file)
            instance_list = os.listdir(sub_dir)
            for instance_name in instance_list:
                self.file_list.append(sub_file + '/' + instance_name)
        ...
        print_log(f'[DATASET] sample out {self.sample_points_num} points', logger = 'Objaverse_8192')
        print_log(f'[DATASET] Load data from {self.data_root}', logger = 'Objaverse_8192')
        print_log(f'[DATASET] aug_type {self.aug_type}', logger = 'Objaverse_8192')
        print_log(f'[DATASET] corrupt_type {self.corrupt_type}', logger = 'Objaverse_8192')
        ...
        #         'file_path': line
        #     })
        print_log(f'[DATASET] {len(self.file_list)} instances were loaded', logger = 'Objaverse_8192')
    ...
    def __getitem__(self, idx):
        ...
        data = IO.get(os.path.join(self.data_root, sample)).astype(np.float32)
        ...
        return 0, 0 , corrupted_data, clean_data
