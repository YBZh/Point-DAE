...
expname_list = [
    # "vis_pretrain_PointCAE_affine_r3_dropout_local",
    # "vis_pretrain_PointCAE_affine_r3",
    # "vis_pretrain_PointCAE_dropout_local",
    # "vis_pretrain_PointCAE_rotate_z",
    # "vis_pretrain_PointCAE_rotate",
    # "vis_pretrain_PointCAE_reflection",
    # "vis_pretrain_PointCAE_scale_nonorm",
    # "vis_pretrain_PointCAE_shear",
    # "vis_pretrain_PointCAE_translate",
    # # affine transformation combination
    # "vis_pretrain_PointCAE_affine_r3_dropout_patch",
    # ####
    # "vis_pretrain_PointCAE_clean",
    # # add noise
    # "vis_pretrain_PointCAE_add_global",
    # "vis_pretrain_PointCAE_add_local",
    # "vis_pretrain_PointCAE_jitter",
    # ### drop points
    # "vis_pretrain_PointCAE_dropout_global",
    # "vis_pretrain_PointCAE_dropout_patch",
    # "vis_pretrain_PointCAE_nonuniform_density",
    ###
    # "vis_pretrain_PointCAE_dropout_local_occow",
    # "vis_pretrain_PointCAE_affine_r3_occow"
    # "vis_pretrain_PointCAE_transformer_dropout_patch_affine_r3_maskpatch_p0005_PointCAE_transformer_with_fc_center_p"
    "vis_pretrain_PointCAE_affine_r3_dropout_local_4xlonger_Point_CAE_DGCNN_FCOnly"
    # "vis_pretrain_PointCAE_transformer_dropout_patch_maskpatch_p0005_PointCAE_transformer_with_fc_center_p",
    # "vis_pretrain_PointCAE_transformer_affine_r3_PointCAE_transformer_with_fc_center_p"
    ]
for expname in expname_list:
    # root = './vis/' + expname + '/02691156_520/'  %%% 500, 520, 225, 234, 296, 298, 66, 77
    root = './vis/' + expname + '/03467517_77/'
    obj_file_name = root + 'pointdae.pth.tar'
    a = torch.load(obj_file_name)
    input_pc = a['input_point'][0]
    coarse = a['coarse'][0]
    fine = a['fine']
    idx = farthest_point_sample(fine, 1024)
    fine = index_points(fine, idx)[0]   #[B, S, C][0]
    gt = a['gt'][0]

    d = {'x': input_pc[:, 0], 'y': input_pc[:, 1], 'z': input_pc[:, 2]}
    cloud = PyntCloud(pd.DataFrame(data=d))
    cloud.to_file(root + 'inputpc.ply')

    d = {'x': coarse[:, 0], 'y': coarse[:, 1], 'z': coarse[:, 2]}
    cloud = PyntCloud(pd.DataFrame(data=d))
    cloud.to_file(root + 'coarse.ply')

    d = {'x': fine[:, 0], 'y': fine[:, 1], 'z': fine[:, 2]}
    cloud = PyntCloud(pd.DataFrame(data=d))
    cloud.to_file(root + 'fine.ply')

    d = {'x': gt[:, 0], 'y': gt[:, 1], 'z': gt[:, 2]}
    cloud = PyntCloud(pd.DataFrame(data=d))
    cloud.to_file(root + 'gt.ply')
