...
from .pointnet_not_util import PointNetEncoderNoT_partseg
...
class Point_CAE_PointNetNoT_PartSeg(nn.Module):
    def __init__(self, config):
        super().__init__()
        print_log(f'[Point_CAE_PointNetNoT_PartSeg] ', logger ='Point_CAE_PointNetNoT_PartSeg')
        self.config = config
...
# finetune model
@MODELS.register_module()
class PointNetNoT_PartSeg(nn.Module):
    def __init__(self, cls_dim, **kwargs):
        super().__init__()
        # self.config = config
    ...
    def get_loss_acc(self, ret, gt):
        ...
        loss = F.cross_entropy(ret, gt.long(), reduction='mean') ## no smooth by default
        pred = ret.argmax(-1)
...
# finetune model
@MODELS.register_module()
class PointNetNoT_Linear_PartSeg(nn.Module):
...
# finetune model
@MODELS.register_module()
class PointNetNoT_feat_PartSeg(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.config = config
        self.cls_dim = config.cls_dim
