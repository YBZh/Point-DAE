        ...
        if 'log' in file or 'pt.txt' in file:
            ...
            with open(fpath, 'r') as f:
                ...
                for line in lines:
                    ...
                    if complete_flag:
                        if args.taskaffinity:
                            if '[Validation] Acc: ' in line:  # for task affinity
                                num = min(float(line.split('= ')[1]), num)
                        else:
                            if '[Validation] EPOCH: ' in line:
                                ...
                                pass
            if args.taskaffinity and num != 100:
                output['task affinity loss:'] = num
            elif args.taskaffinity and num == 100:
                complete_flag = False
            elif not args.taskaffinity and num != 0:
                output['val acc:'] = num
            else:
                complete_flag = False
            if complete_flag:
...
def parse_function_fewshot(*metrics, directory='', args=None, end_signal=None):
    ...
    for file in file_list:
        if 'log' in file or 'pt.txt' in file:
            ...
            if way != 'None' and shot != 'None' and num != 0:
                outputs.append(output)
        else:
            pass
...
def main(args, end_signal):
    ...
    if args.few_shot:
        ...
        if args.multi_exp:
            ...
            for directory in listdir_nohidden(args.directory, sort=True):
                process_flag = True
                if len(args.name_required) != 0:
                    for required_name in args.name_required:
                        if required_name not in directory:
                            process_flag = False
                            break
                    if not process_flag:
                        continue
                if len(args.name_forbidded) != 0:
                    for forbid_name in args.name_forbidded:
                        if forbid_name in directory:
                            process_flag = False
                            break
                    if not process_flag:
                        continue
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--name_required', nargs='+', default=[]
    )
    parser.add_argument(
        '--name_forbidded', nargs='+', default=[]
    )
    parser.add_argument(
        '--taskaffinity', action='store_true', help='parse multiple experiments'
    )
    parser.add_argument(
